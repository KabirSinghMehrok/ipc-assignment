...
int main()
{
	...
	strcpy(socket_add.sun_path, "/tmp/mysocket");
	...
	int connect_val = connect (socket_val, (const struct sockaddr *) &socket_add, sizeof(struct sockaddr_un));
	
	if (connect_val == -1)	
	{
		fprintf(stderr, "ERROR: Socket connection with P2 has failed\n");
		exit(1);
	}
	...
	for (int i=0; i<string_no; i += 5)
	{
		...
		for (int j=0; j<5; j++)
		{
			...
			int send_message_val =  write(socket_val, string_arr[i+j], string_size + 3);
		...
		int recieve_message_val = read(socket_val, msg_ptr, 3 * sizeof(char));
	...
	int close_val = close(socket_val);

	if (close_val == -1)
	{
		fprintf(stderr, "ERROR: Socket not properly closed\n");
		exit(1);
	}
